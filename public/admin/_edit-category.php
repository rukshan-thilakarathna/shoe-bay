...
function imgUp($imgArr,&$e1,&$e2,&$e3,&$e4,&$e5,$fileName="") {
	...
	*/		
			...
			if(!in_array(TRUE, $err) && $r['uz']!=$zyuz){
			...
			if(!in_array(TRUE, $err)){
				if(!empty($r['iz']) && !empty($iz)) unlink('../img/categories/'.$r['iz']);
				$iz = empty($iz) ? $r['iz'] : $iz;
				//-- generated by robotcoding2_SEYOL --//
				$OK = $db->prepare("UPDATE `categories` SET `cn`=?, `cd`=?, `od`=?, `mn`=?, `uz`=?, `az`=?, `mt`=?, `md`=?, `iz`=? WHERE `cid`=?")->execute([$zycn, $zycd, $zyod, $zymn, $zyuz, $zyaz, $zymt, $zymd, $iz, $_POST['zyid']]);
			}
		}else{
 ...
 ?>
	<div id="zy1">
		<?php if($OK) echo '<div class="x3ok">The Category has been updated successfully.</div>'; ?>
		<h1>Edit Category</h1>
