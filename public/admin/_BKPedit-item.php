...
function imgUp($imgArr,&$e1,&$e2,&$e3,&$e4,&$e5) {
	$img="";
	/*
	e1 = uploading error
	...
	*/
	...
	if(in_array($ext,array("jpg","jpeg","png","gif"))){
		...
		// too small image
		if($imd[0]<480 || $imd[1]<50) $e2=TRUE;
		elseif($imgArr["size"] > 2097152) $e3=TRUE;
		else{
			if($ext=='jpeg') $ext = 'jpg';
			$img = date('Ymdhis').mt_rand().".".$ext;
			$l = "../up/".$img;
			$ok = move_uploaded_file($imgArr["tmp_name"],$l);
			if($ok){
				$h = round((480*$imd[1])/$imd[0]);
				$h = $h>170 ? 170 : $h;
				imgz($ext,480,$h,$l);
			}else $e5=TRUE;
	...
	*/		
			...
			if(!in_array(TRUE, $err)){
				...
				//-- generated by robotcoding2_SEYOL --//
					$OK = $db->prepare("UPDATE `items` SET `na`=?, `cd`=?, `ca`=?, `pr`=?, `vo`=?, `pe`=?, `co`=?, `br`=?, `mt`=?, `md`=?, `iz`=?, `az`=? WHERE `idz`=?")->execute([$zyna, $zycd, $zyca, $zypr, $zyvo, $zype, $zyco, $zybr, $zymt, $zymd, $iz, $zyaz, $_POST['zyid']]);
			}
		}else{
			...
			<label class="x3lb">
				...
				<div class="x3d">
					<select class="x3in" name="zyca" id="zyca">
						<?php 
							echo SE_optionsGen($arr_cat,$zyca); 
						?>
					</select>
					<span class="x3s1" id="zyca0"></span>
				</div>
			</label>
			<label class="x3lb">
				...
				<div class="x3d">
					...
					<span class="x3s1" id="zypr0"></span>
				</div>
			</label>
			<!--<div class="x3lb">
				<span class="x3s">Image</span>
				<div class="x3d">
					<input type="file" class="x3in" name="zyiz" id="zyiz">
					<span class="x3s1" id="zyiz0"></span>
				</div>
			</div>-->
			<label class="x3lb">
