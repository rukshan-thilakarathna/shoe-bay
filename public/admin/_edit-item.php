	...
	*/		
			...
			if(!in_array(TRUE, $err)){
				...
				//-- generated by robotcoding2_SEYOL --//
					$OK = $db->prepare("UPDATE `items` SET `na`=?, `cd`=?, `ca`=?, `pr`=?, `vo`=?, `pe`=?, `co`=?, `br`=?, `mt`=?, `md`=?, `iz`=?, `az`=?, `uz`=? WHERE `idz`=?")->execute([$zyna, $zycd, $zyca, $zypr, $zyvo, $zype, $zyco, $zybr, $zymt, $zymd, $iz, $zyaz, $zyuz, $_POST['zyid']]);
			}
		}else{
