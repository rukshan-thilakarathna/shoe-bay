	...
	*/		
			...
			if(!in_array(TRUE, $err)){
				...
				//-- generated by robotcoding2_SEYOL --//
					$OK = $db->prepare("UPDATE `items` SET  `colour`=?, `size`=?, `desr`=?,`na`=?, `cd`=?, `ca`=?,`sid`=?,`ssid`=?, `pr`=?, `vo`=?, `pe`=?, `co`=?, `br`=?, `mt`=?, `md`=?, `iz`=?, `az`=?, `uz`=? WHERE `idz`=?")->execute([$colour1,$size1,$desr1,$zyna, $zycd, $zyca,$zysubca,$zysubsubca, $zypr, $zyvo, $zype, $zyco, $zybr, $zymt, $zymd, $iz, $zyaz, $zyuz, $_POST['zyid']]);
			}
		}else{
	...
	<div id="zy1">
		...
		<fieldset class="x3fs">
			...
			<label class="x3lb" id="Colour" >
				...
				<?php
					foreach(explode(', ', $r['colour']) as $key => $color){
				?>
					<div class="x3d" id="maindiv<?php echo $key.'update' ?>"  style="display:  flex;margin-top:12px">
						<input type="text" class="x3in" name="colour[]" value="<?php echo $color; ?>" onkeyup="showResult(this.value,'<?php echo $key.'update' ?>')"  id="color_input_<?php echo $key.'update' ?>" style="width: 50%;">
						<div style="width: 50%;background: <?php echo $color; ?>;border-radius:  5px;margin-left: 7px;" id="color_div_<?php echo $key.'update' ?>"></div>
						<button onclick="AddRow()" type="button" class="x3in x3in2" style="margin: 0 0 0 6px;width: 165px;">Add New</button>
						<button onclick="removeRow('maindiv<?php echo $key.'update' ?>')" type="button" class="x3in x3in2" style="margin: 0 0 0 6px;width: 50px;">X</button>
					</div>
				<?php	
					}
				?>
			...
			<label class="x3lb" id="Size" >
				...
				<?php
					foreach(explode(', ', $r['size']) as $key => $size){
				?>
					<div class="x3d" id="maindivd<?php echo $key.'update' ?>" style="display: flex;margin-top:12px">
						<input type="text" class="x3in" name="size[]" value="<?php echo $size; ?>" onkeyup="showResultSize(this.value,<?php echo $key.'update' ?>)"   style="width: 103%;">
						<button onclick="AddRowSize()" type="button" class="x3in x3in2" style="margin: 0 0 0 6px;width: 165px;">Add New</button>
						<button onclick="removeRow('maindivd<?php echo $key.'update' ?>')" type="button" class="x3in x3in2" style="margin: 0 0 0 6px;width: 50px;">X</button>
					</div>
				<?php	
					}
				?>
			...
			<label class="x3lb">
				...
				<div class="x3d">
					<textarea name="des" id="zycd" class="x3in"><?php echo $r['desr'] ?? ''; ?></textarea>
				</div>
			</label>
